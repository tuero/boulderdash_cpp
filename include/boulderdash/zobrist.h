#ifndef BOULDERDASH_ZOBRIST_H_
#define BOULDERDASH_ZOBRIST_H_
...
namespace std {
template <std::size_t Words>
struct hash<boulderdash::ZobristKey<Words>> {
    auto operator()(const boulderdash::ZobristKey<Words>& key) const noexcept -> std::size_t {
        uint64_t h = 0;
        for (std::size_t i = 0; i < Words; ++i) {
            h ^= boulderdash::splitmix64(key.word[i] + boulderdash::SPLIT64_C1 * static_cast<uint64_t>(i + 1));
        }
